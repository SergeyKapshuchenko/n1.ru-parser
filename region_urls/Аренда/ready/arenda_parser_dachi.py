...
write_to = "Дачи.csv"
read_from = "../urls/arenda_dachi.csv"
...
def parse_single_url(url, region):
    ...
    try:
        for value in response.xpath("//li[@class='card-dacha-content-params-list__item']"):
            text = value.text
            if "Площадь" in text:
                area = text.split('участка')[1].split()[0]
                if "," in area:
                    area = float(area.replace(",", ".")) * 100
                    area = int(area)
                else:
                    area = int(area) * 100
            elif "Этаж" in text:
